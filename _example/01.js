...
// 1.부서별 평균월급여 
let avgSalary = new Array(dData.length).fill(0);
for(let i = 0; i < dData.length; i++){
  let cnt = 0;
  for(let j = 0; j < eData.length; j++){
    if(dData[i].no === eData[j].dno){
      avgSalary[i] += parseInt(eData[j].salary);
      cnt++;
    }
  }
  if(cnt === 0){
...
console.log('문제 2: ' + empCnt + ' ' + comCnt);
